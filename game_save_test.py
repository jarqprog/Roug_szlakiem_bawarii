...
################ TU URUCHAMIAMY FUNKCJE:
    ...
    print("nie wpisuj walka, tylko kliknij enter, losuj przedmioty (3 krotnie losuje - TMP)")
    event_choose(hero = hero, location = hero.location)
    print("robimy zapis bohatera do pliku")
    mod_save_load.export_hero(hero = hero, filename="autosavehero.txt")
    hero.name = "Jarosław Kucharczyk"
    hero.location = 3
    hero.actualExp = 10000
    hero.maxLife = 99999999
    print("nadapłem nowe staty postaci...(imię, lokację, exp, life...)")
    print("zmienione właściwości bohatera, statystyki po zmianach "), dot_loop()
    pause()
    display_hero_chart(hero = hero)
    print("pociśnij entery, żeby wylosować do ekwipunku nowe rzeczy..")
    pause()
    hero.name = input(str("wpisz nowe imię bohatera "))
    hero.proffession = input(str("wpisz nową klasę bohatera "))
    print("po zmianach"), dot_loop()
    pause()
    display_hero_chart(hero = hero)
    print("znów polosuj przedmioty żeby narobić zmian...")
    event_choose(hero = hero, location = hero.location)
    print("robimy load game :)"), dot_loop()
    pause()
    hero = mod_save_load.import_hero(importfilename="autosavehero.txt")
    print("zaraz wyświetlimy postać z save'a :) ")
    pause()
    display_hero_chart(hero = hero)
    a = input(str("podaj nazwę pliku do save..."))
    if a == '': a = "nazwa Łukasz.."
    mod_save_load.export_hero(hero = hero, filename=a)
    print("masz w folderze nowy plik o tej nazwie:)")
    pause()
    print("znów reset postaci...")
    hero.name = 'X'
    hero.proffession = "pupa"
    hero.attrib_dict = {"siła":0, "zwinność":0, "percepcja":0, "inteligencja":0, "siła woli":0}
    hero.inventory_dict = {}
    pause()

    display_hero_chart(hero = hero)

    print("wczytajmy grę z Twojego pliku...")
    hero = mod_save_load.import_hero(importfilename=a)
